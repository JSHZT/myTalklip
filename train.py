...
class Talklipdata(object):
    ...
    def readtext(self, path):
        ...
        #     # trgt = f.readline()[7:]
        trgt = np.load(path, allow_pickle=True).item().strip().strip(',')
...
def collate_fn(dataBatch):
    ...
    Returns:
        inpBatch: input T_sum*6*H*W, concatenation of all video chips in the time dimension
        gtBatch: output T_sum*3*H*W
        inputLenBatch: bs
        audioBatch: bs*104*T'
        audio_idx: T_sum
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_root", help="Root folder of filelists", required=True, type=str)
    ...
    parser.add_argument('--image_size', help='size of input data', default=96, type=int)
